...
def parse_args():
    ...
    p.add_argument("--output",           default=f"{data_root}/data/cot_responses.jsonl")
...
def main():
    ...
    def write(item: dict) -> bool:
        ...
        out_f.write(json.dumps({
            ...
            "confidence": (
                parse_confidence(item["response"])
                or parse_confidence(item.get("thinking", ""))
            ),
    ...
    # ── 4. Summary ────────────────────────────────────────────────
    new_written = written - resume_count
    total_seen = new_written + n_rejected
    pass_rate = new_written / total_seen * 100 if total_seen else 0
    ...
    print(
        ...
        f"(resume base: {resume_count}, this run: {new_written})"
    )
    print(
        f"Rejected by filter: {n_rejected}  "
        f"pass rate: {pass_rate:.1f}%  (this run only)"
    )
